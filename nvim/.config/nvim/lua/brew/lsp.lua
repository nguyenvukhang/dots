...
nvim_lsp.tsserver.setup({
	on_attach = function(_, bufnr)
		on_attach(_, bufnr)
		set_keymap(bufnr, "n", "<leader>p", ":silent w<CR>:silent !prettier --write %<CR>", opts)
	end,
...
require("lspconfig").ccls.setup({
	on_attach = function(_, bufnr)
		on_attach(_, bufnr)
		set_keymap(bufnr, "n", "<leader>p", ":silent w<CR>:silent !clang-format -i %<CR>", opts)
	end,
	capabilities = capabilities,
})
...
nvim_lsp.sumneko_lua.setup({
	on_attach = function(_, bufnr)
		on_attach(_, bufnr)
		set_keymap(bufnr, "n", "<leader>p", ":silent w<CR>:silent !stylua %<CR>", opts)
	end,
	capabilities = capabilities,
	settings = {
		Lua = {
